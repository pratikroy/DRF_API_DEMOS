...
class ArticleSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        """
        Checks whether title and desctiption are different or not
        """
        if data["title"] == data["description"]:
            raise serializers.\
            ValidationError("title and description must be different")
        return data
    ...
    def validate_title(self, value):
        if len(value) < 20:
            raise serializers.\
            ValidationError("Title must be at least 20 chars long")
        return value
